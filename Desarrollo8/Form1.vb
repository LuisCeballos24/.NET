...
Public Class Form1
    Private Sub btnCalcular_Click(sender As Object, e As EventArgs) Handles btnCalcular.Click
        ...
        ElseIf rdbSerie.Checked = True Then
            Do
                iteracionActual += 1
                Dim termino1 As Double = -2 * Math.Log10((rugosidadRelativa / (3.7 * diametroInterior)) + (2.51 / (gravedad ^ 0.5 * diametroInterior ^ 0.5 * factorFriccion ^ 0.25)))
                Dim termino2 As Double = (1 / factorFriccion) ^ 0.5
                factorFriccion = 1 / ((termino1 ^ 2 + termino2 ^ 2) ^ 0.5)
                If iteracionActual > numeroMaximoIteraciones Then
                    MsgBox("No se pudo calcular el factor de fricción después de " & iteracionActual & " iteraciones.", MsgBoxStyle.Critical, "Error")
                    Exit Sub
                End If
            Loop While factorFriccion > factorFriccionMaximo

            ' Calcula la pérdida de carga '
            Dim perdidaCarga As Double = (factorFriccion * longitudTotal / diametroInterior) * (caudalMaximo ^ 2 / (2 * gravedad))

            ' Muestra el resultado al usuario '

            MsgBox("La perdida de carga en la tuberia en serie es: " & perdidaCarga.ToString("#,##0.00") & " m")
        ...
        ElseIf rbdMixta.Checked = True Then
            ...
            presion_disponible(0) = longitudEquivalent1
            ...
            presion_disponible(1) = longitudEquivalent2
            ...
            ' Agregar los datos al chart
            Chart1.Series.Add("Longitud equivalente vs. pérdida de carga")
            Chart1.Series(0).ChartType = SeriesChartType.Line
            For i As Integer = 0 To num_tuberias - 1
                Chart1.Series(0).Points.AddXY(presion_disponible(i), perdida_caida_presion(i))
            Next
            ...
            ' Configurar los ejes
            Chart1.ChartAreas(0).AxisX.Title = "Longitud equivalente (m)"
